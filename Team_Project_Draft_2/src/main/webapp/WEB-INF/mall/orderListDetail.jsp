...
section.cart p, section.cart span {
	margin: 0;
}
...
<section class="cart">
	...
	<table class="cart__list">
		...
		<tbody>
			<c:forEach var="item" items="${payResultDetailList}">
				<tr>
					<td>${item.onum}</td>
					<td><fmt:formatDate var="formatDate" value="${item.orderdate}"
							pattern="yyyy-MM-dd HH:mm:ss" />${formatDate}</td>
					<td>${item.carname}</td>
					<td><c:set var="images" value="${fn:split(item.cimage,',')}" />
						<img id="img_car"
						src="<%=request.getContextPath()%>/resources/images/${images[0]}">
						<br /></td>
					<td><a class="com-link"
						href="orderDetail.mall?cnum=${item.cnum}">주문 상세보기</a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	<div class="cart__mainbtns" align="center">
		<button class="cart__bigorderbtn right" onClick="goShop()">쇼핑
			계속하기</button>
	</div>
